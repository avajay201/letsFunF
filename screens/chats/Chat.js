...
const Chat = ({ navigation }) => {
  ...
  const [imageSenderQueue, setImageSenderQueue] = useState([]);
  let chatName;
  ...
  useEffect(() => {
    ...
    if ((!userProfile.blocked && chatWS.current) || (chatOn && chatWS.current)){
      return;
    }
  ...
  const renderMessage = ({ item }) => {
    ...
    return (
      <>
        {/* Message */}
        <TouchableOpacity onLongPress={handleMessageDeleteModal}>
          <Animatable.View
            animation='flipInX'
            duration={300}
            style={[
          ...
          >
            <View>
              {msgType === 'Text' ? (
                  ...
                  <TouchableOpacity onLongPress={handleMessageDeleteModal} onPress={() => handleMessageImagePreview(BASE_URL + item.image)}>
                  <Image 
  ...
  // Image send queue start
  useEffect(()=>{
    const processQueue = async()=>{
      if (imageSenderQueue.length > 0 && chatWS.current){
        const currentImage = imageSenderQueue[0];
        const data = await FileSystem.readAsStringAsync(currentImage.uri, {
  ...
  const sendMessage = async() => {
    ...
    try {
      ...
      if (selectedMedia.length > 0 && chatWS.current) {
        setImageSenderQueue(selectedMedia);
      }
  ...
  const userBlock = async() => {
    setConfirmModalVisible(false);
    setIsLoading(true);
    const response = await blockUser({ user: userName, status: userProfile.other_blocked ? 'unblock' : 'block' });
    if (response[0] === 200) {
      if (chatWS.current){
        const message = {
          status: 'block',
      ...
      setTimeout(()=>{
        ...
        ToastAndroid.show(`User ${userProfile.other_blocked ? 'unblocked' : 'blocked'}!`, ToastAndroid.SHORT);
        setIsLoading(false);
      }, 1000);
    } else if (response[0] === 401) {
      ToastAndroid.show('Session expired, please login.', ToastAndroid.SHORT);
      ...
      ToastAndroid.show(`Failed to ${userProfile.other_blocked ? 'unblock' : 'block'} user!`, ToastAndroid.SHORT);
    }
    setIsLoading(false);
  };
  // Block user end
  ...
  const pickMedia = async () => {
    ...
    if (permissionResult.granted === false) {
      ToastAndroid.show('Permission to access media library is required!', ToastAndroid.SHORT);
      return;
    }
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <View style={styles.profileContainer}>
          <Avatar src={userProfile.blocked ? require('../../assets/profile.png') : (userProfile.profile_picture ? BASE_URL + userProfile.profile_picture : null)} name={userName} is_url={userProfile.blocked ? false : true} />
          {chatOn && !userProfile.other_blocked && <View style={userStatus ? styles.onlineDot : styles.offlineDot} />}
        </View>
        ...
        <View style={styles.iconContainer}>
          <TouchableOpacity onPress={handleAudioCall} style={styles.callIcon}>
            <MaterialIcons name='call' size={24} color='#000' />
          </TouchableOpacity>

          <TouchableOpacity onPress={handleVideoCall} style={styles.callIcon}>
            <MaterialIcons name='videocam' size={24} color='#000' />
          </TouchableOpacity>

          <TouchableOpacity onPress={toggleModal} style={styles.menuIcon}>
        ...
        >
          <View style={styles.modalContainer}>
            ...
            <TouchableOpacity onPress={() => handleMenuOption('block')}>
              <Text style={styles.modalOption}>{userProfile.other_blocked ? 'Unblock user' : 'Block User'}</Text>
            </TouchableOpacity>
            <View style={styles.separator} />
          ...
          <View style={styles.confirmModalContainer}>
            <Text style={styles.confirmModalHeading}>
              {confirmModalVisible === 'clear'
                ? 'Clear this chat?'
                : userProfile.other_blocked ? 'Unblock this user?' : 'Block this user?'}
            </Text>
            ...
            <View style={styles.confirmButtonContainer}>
              ...
              >
                <Text style={styles.confirmClearText}>
                  {confirmModalVisible === 'clear' ? 'Clear chat' : userProfile.other_blocked ? 'Unblock' : 'Block'}
                </Text>
              </TouchableOpacity>
            </View>
          </View>
      ...
      {/* Selected media */}
      {selectedMedia.length > 0 && !messageSendingLoader && <ScrollView horizontal={true} showsHorizontalScrollIndicator={false} style={styles.imagePreviewContainer} contentContainerStyle={{alignItems: 'center'}}>
        {selectedMedia.map((imageUri, index) => (
          <View key={index} style={styles.selectedImageContainer}>
            <Image source={{ uri: imageUri?.uri }} style={styles.selectedImage} />
            <TouchableOpacity style={styles.cancelButton} onPress={() => deselectMedia(imageUri?.uri)}>
      ...
      {chatOn && !userProfile.other_blocked && (
        <>
          <TextInput
            ...
            placeholderTextColor="#999"
            editable={selectedMedia.length > 0 || messageSendingLoader ? false : true}
          />

          {/* Button for picking media */}
          <TouchableOpacity onPress={pickMedia} style={styles.mediaButton} disabled={newMessage != '' || messageSendingLoader ? true : false }>
            <MaterialIcons name="image" size={45} color={newMessage || messageSendingLoader ? '#d3d3d3' : '#009387'} />
          </TouchableOpacity>

          <TouchableOpacity
            disabled={messageSendingLoader ? true : (selectedMedia.length === 0 ? (!newMessage ? true : false) : false)}
            onPress={sendMessage}
            style={[
              styles.sendButton,
              { backgroundColor: messageSendingLoader ? '#d3d3d3' : (newMessage || selectedMedia.length > 0 ? '#009387' : '#d3d3d3') },
            ]}
          >
